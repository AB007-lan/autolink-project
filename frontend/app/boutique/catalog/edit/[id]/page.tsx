...
export default function EditBoutiqueProductPage() {
  ...
  useEffect(() => {
    if (product && !loaded) {
      ...
      setForm({
        ...
        reference: p.reference ?? p.oemReference ?? '',
        compatibleBrands: p.compatibilities?.map((c: any) => c.brand).filter(Boolean) ?? [],
        compatibleModels: p.compatibilities?.map((c: any) => c.model).filter(Boolean).join(', ') ?? '',
      });
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="Ex : Corolla, Camry, Yaris"
              className="w-full px-4 py-3 border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
            />
          </div>
        </div>
